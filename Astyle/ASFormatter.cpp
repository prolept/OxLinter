...
string ASFormatter::nextLine()
{
	...
	//add  Oxcleaner	else if (currentChar == '"' || currentChar == '\'')
		else if (currentChar == '"')
		{
			formatQuoteOpener();
			testForTimeToSplitFormattedLine();
		...
		// Check if in template declaration, e.g. foo<bar> or foo<bar,fig>
			...
			//add  Oxcleaner (!isInTemplate && currentChar == '<') 
	   if (!isInTemplate && currentChar == '<')
		...
		// handle parens
		//add  Oxcleaner if (currentChar == '(' || currentChar == '[' || (isInTemplate && currentChar == '<'))
				if (currentChar == '<' ||currentChar == '(' || currentChar == '[' || (isInTemplate ))
		{
			...
			if (currentChar == '[')
				...
				//add  Oxcleaner else if (currentChar == ')' || currentChar == ']' || (isInTemplate && currentChar == '>'))
	 else if (currentChar == '>'|| currentChar == ')' || currentChar == ']' || (isInTemplate))
		{
			foundPreCommandHeader = false;
			parenStack->back()--;
			...
			//add  Oxcleaner if (isInTemplate && currentChar == '>')
			if (isInTemplate  )
			{
				templateDepth--;
				if (templateDepth == 0)
...
bool ASFormatter::isUnaryOperator() const
{
	...
	return ((isCharImmediatelyPostReturn || !isLegalNameChar(previousCommandChar))
	        && previousCommandChar != '.'
	        && previousCommandChar != '\"'
	      //add  Oxcleaner  && previousCommandChar != '\''
	        && previousCommandChar != ')'
	        && previousCommandChar != ']');
}
...
void ASFormatter::padOperators(const string* newOperator)
{
	...
	bool shouldPad = (newOperator != &AS_SCOPE_RESOLUTION
		///
	/*	&& newOperator != &OX_DOT_NOTEQUAL
		&& newOperator != &OX_DOT_LE
		&& newOperator != &OX_DOT_GR
		&& newOperator != &OX_DOT_EQUAL*/
		//
...
//x                        && isPointerOrReference())
	                  ////&& !(newOperator == &AS_MULT   ////add  Oxcleaner
	                  ////     && (previousNonWSChar == '.'
		             ...
		             && !(newOperator == &AS_LS  ///// add  Oxcleaner	<>
	                  ...
	                  && !isInAsmOneLine
	                  && !isInAsmBlock
					   && !(newOperator == &AS_AND  ///// add  Oxcleaner					.&&
	                       && (previousNonWSChar == '.'))    // check for ->
					    && !(newOperator == &AS_OR  ///// add  Oxcleaner					.||
	                       && (previousNonWSChar == '.'))    // check for ->
						&& !(newOperator == &AS_DIV  /////add  Oxcleaner						./
	                       && (previousNonWSChar == '.'))    // check for ->
					&& !(newOperator == &AS_BIT_XOR  /////add  Oxcleaner						 
	                       && (previousNonWSChar == '.'))    // check for ->					.^
						  && !(newOperator == &AS_MULT  ///// add  Oxcleaner					 
	                       && (previousNonWSChar == '.'))    // check for ->					.*

		   && !(newOperator == &AS_LS  ///// add  Oxcleaner	 2				 
	                       && (previousNonWSChar == '.'))    // check for ->					.<
		   && !(newOperator == &AS_GR  ///// add  Oxcleaner	2				 
	                       && (previousNonWSChar == '.'))    // check for ->					.>
		  && !(newOperator == &AS_NOT  ///// add  Oxcleaner	2				 
	                       && (previousNonWSChar == '.'))    // check for ->					.!

		&& !(newOperator == &AS_LS_EQUAL && (previousNonWSChar == '.'))  //ox .<= 
		&& !(newOperator == &AS_GR_EQUAL && (previousNonWSChar == '.'))  //ox .>=
		&& !(newOperator == &AS_EQUAL && (previousNonWSChar == '.'))  //ox .==
		 		 //&& !(newOperator == &AS_GR  ///// add  Oxcleaner	2				 
	  //                     && (previousNonWSChar == '<'))    // check for ->					<> //matrix
							
						 && !(newOperator ==&AS_ASSIGN && previousNonWSChar == '[')/// add  Oxcleaner lambda function  [=
		                 && !(newOperator == &AS_QUESTION && (previousNonWSChar == '.'))		/// add  Oxcleaner conditional .?
						 && !(newOperator == &AS_COLON && (previousNonWSChar == '.'))		 /// add  Oxcleaner conditional  .:
	                 );
		...
		if (shouldUnPadParens)
		{
			...
			if (shouldPadParensOutside || prevIsParenHeader)
				spacesOutsideToDelete--;
			        ...
			        /* || (lastChar == '>' && !foundCastOperator)  add  Oxcleaner
			         || lastChar == '<'*/
			         || lastChar == '?'
			         || lastChar == ':'
			         || lastChar == ';'
...
{
	//malcik assert(currentChar == '"' || currentChar == '\'');
	 assert(currentChar == '"' );
	isInQuote = true;
...
size_t ASFormatter::findNextChar(string &line, char searchChar, int searchStart /*0*/)
{
	...
	//add  Oxcleaner 	if (line[i] == '\'' || line[i] == '\"')
		if ( line[i] == '\"')
		{
			char quote = line[i];
			while (i < line.length())
...
bool ASFormatter::isStructAccessModified(string &firstLine, size_t index) const
{
	...
	while (sourceIterator->hasMoreLines() || isFirstLine)
	{
		...
		for (size_t i = 0; i < nextLine_.length(); i++)
		{
			...
			//add  Oxcleaner 	if (nextLine_[i] == '"' || nextLine_[i] == '\'')
			if (nextLine_[i] == '"' )
			{
				isInQuote_ = true;
...
void ASFormatter::checkIfTemplateOpener()
{
	...
	////
	// add  Oxcleaner pas de template ds ox
	isInTemplate = false;
		return;
		////
	...
	while (sourceIterator->hasMoreLines() || isFirstLine)
	{
		...
		/// add  Oxcleaner	if (currentChar_ == '"' || currentChar_ == '\'')
			if (currentChar_ == '"'  )
			{
				isInQuote_ = true;
...
void ASFormatter::updateFormattedLineSplitPoints(char appendedChar)
{
	...
	if (appendedChar == '[' || appendedChar == ']'
	        ...
	        || nextChar == '[' || nextChar == ']')
		return;
	//// don't split before or after a block paren add  Oxcleaner
	if (appendedChar == '<' || appendedChar == '>'
	        || previousNonWSChar == '<'
	        || nextChar == '<' || nextChar == '>')
		return;
	...
	{
		//add  Oxcleaner if (nextChar != ')' && nextChar != '(' && nextChar != '"' && nextChar != '\'')
